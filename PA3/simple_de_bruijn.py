...
def read_assembly_reads(read_fn):
    ...
    output_reads = [_[0] for _ in reads]+[_[1] for _ in reads]
...
def simple_de_bruijn(sequence_reads, k):
    ...
    return de_bruijn_graph, out_degree, in_degree


def de_bruijn_reassemble(de_bruijn_graph, out_degree, in_degree):
    """
    ...
    while True:
        ...
        if n_values == 0:
            break
        good_starts = [k for k in de_bruijn_graph if de_bruijn_graph[k] and out_degree[k] > in_degree[k]]
...
if __name__ == "__main__":
    ...
    db_graph, out_degree, in_degree = simple_de_bruijn(reads, 25)
    for k in db_graph.keys()[:40]:
        print k, db_graph[k]

    output = de_bruijn_reassemble(db_graph,out_degree,in_degree)
